...
void SystemClock_Config96(void);
void SystemClock_Config480(void);
...
int main(void)
{
  ...
  static char buffer_uart[128];
  /* USER CODE END 2 */

  SCB_EnableICache();
  SCB_EnableDCache();
  delta_ms = testSpeed();
  int size_cdc = sprintf(buffer_cdc, "USB. Speed %i ms", (int)delta_ms);
  int size_uart = sprintf(buffer_uart, "UART. Speed %i ms", (int)delta_ms);
  ...
  while (1)
  {
	...
	HAL_UART_Transmit(&huart3, (uint8_t*)buffer_uart, size_uart, 1000);
	HAL_Delay(2000);
